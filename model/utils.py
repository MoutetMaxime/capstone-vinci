...
def get_routing(start_point, end_point, delay=0):
    ...
    params = {
        ...
        "start": f"{start_point[1]},{start_point[0]}",  # New start coordinates
        "end": f"{end_point[1]},{end_point[0]}",  # New end coordinates
        "profile": "car",
    ...
    # Check response
    tries = 3
    while response.status_code != 200 and tries > 0:
...
def construct_adjacency(nodes):
    ...
    for i, node_i in enumerate(nodes):
        for j, node_j in enumerate(nodes):
            if i == j:
                continue
            adjacency_matrix[i, j] = get_routing(
